...
public class Messages {
	...
	public Messages(Object[] sendee, String message) {
		...
		switch(message) {
			...
			case "PlayersOnly":
				((CommandSender)sendee[0]).sendMessage("Ezt csak játékos használhatja!");
				break;
			...
			case "TeleportRequestCancelled-b":
				((CommandSender)sendee[1]).sendMessage(ChatColor.GOLD + ((CommandSender)sendee[0]).getName() + ChatColor.RED + " has cancelled his/her teleport-request.");
				break;
			case "TeleportRequestTimeout":
				((CommandSender)sendee[0]).sendMessage(ChatColor.RED + "Teleport-request sent to " + ChatColor.GOLD + ((CommandSender)sendee[1]).getName() + ChatColor.RED + " has expired.");
				((CommandSender)sendee[1]).sendMessage(ChatColor.GOLD + ((CommandSender)sendee[0]).getName() + "'s " + ChatColor.RED + "teleport-request has expired.");
				break;
			...
			case "TeleportSuccesful":
				...
				break;
			case "CannotTeleportSafely":
				((CommandSender)sendee[0]).sendMessage(ChatColor.RED + "Cannot teleport safely to " + ChatColor.GOLD + ((CommandSender)sendee[1]).getName());
				((CommandSender)sendee[1]).sendMessage(ChatColor.GOLD + ((CommandSender)sendee[0]).getName() + ChatColor.RED + " cannot teleport safely to you");
				break;
